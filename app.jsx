...
const CHANGELOG = [
  {
    ...
    tagColor: "#dc2626", items: [
      ...
      { cat: "new", text: { th: "การ์ด 'สุขภาพระบบ' ในแท็บความปลอดภัย — เช็คสด: Supabase Auth (ล็อกอิน) · ฐานข้อมูล · เว็บ GitHub Pages · workflow กัน Supabase หลับ · workflow build/deploy · จำนวนรหัสผ่านหมดอายุ/ใกล้หมด รวมทุกอย่างที่ทำให้เข้าระบบไม่ได้ไว้จอเดียว", en: "'System Health' card in the Security tab — live checks: Supabase Auth (login) · database · GitHub Pages site · keepalive workflow · build/deploy workflow · expired/expiring password counts — everything that can block logins, on one screen" } },
      { cat: "new", text: { th: "Health check อัตโนมัติทุกวัน (GitHub Actions) — ping กัน Supabase หลับ + เช็ค Auth/เว็บ ถ้าพังจะเปิด Issue แจ้งเตือนอัตโนมัติ (GitHub ส่งอีเมลถึงเจ้าของ repo)", en: "Daily automated health check (GitHub Actions) — Supabase keepalive ping + Auth/site checks; on failure it opens an alert Issue automatically (GitHub emails the repo owner)" } },
      { cat: "fix", text: { th: "เมนู dropdown เด้งไปโผล่คนละที่กับช่อง (เช่น ตัวกรองในหน้าการชำระเงิน) — ย้ายเมนูไป render ที่ body ตรง ๆ กันพิกัดเพี้ยนจาก transform/backdrop-filter ของการ์ดแม่ มีผลกับ dropdown ทุกตัวทั้งแอป", en: "Dropdown menus appeared far from their field (e.g. payment filters) — menus now render directly under <body>, immune to parent card transform/backdrop-filter coordinate bugs; applies to every dropdown app-wide" } },
