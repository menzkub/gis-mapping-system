...
function AuthScreen({ initialError }) {
  ...
  return (
    <div className="auth-root" style={{ height: "100vh", display: "grid", gridTemplateColumns: "1.1fr 1fr", overflow: "hidden", background: "var(--bg)" }}>
      ...
      {/* Right — form */}
      <div className="auth-form-panel" style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 40, overflow: "auto" }}>

        {/* Mobile-only brand header */}
        <div className="auth-mobile-brand" style={{ display: "none", width: "100%", padding: "48px 24px 0" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
            <div style={{ width: 52, height: 52, borderRadius: 16, background: "linear-gradient(135deg,#f47b20,#ffba7a)", display: "grid", placeItems: "center", boxShadow: "0 10px 28px rgba(244,123,32,0.5)", flexShrink: 0 }}>
              <Icon name="bolt" size={26} stroke={2.4} />
            </div>
            <div>
              <div style={{ fontSize: 11, letterSpacing: "0.18em", fontWeight: 700, color: "#ffba7a", textTransform: "uppercase" }}>PEA</div>
              <div style={{ fontSize: 17, fontWeight: 800, color: "white", letterSpacing: "-0.01em" }}>Meter &amp; TR Search</div>
            </div>
          </div>
          <div style={{ fontSize: 12, fontWeight: 700, color: "#d4abff", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 6 }}>ระบบค้นหา · กฟอ.ฝาง</div>
          <div style={{ fontSize: 26, fontWeight: 800, color: "white", lineHeight: 1.2, marginBottom: 6, letterSpacing: "-0.02em" }}>
            ค้นหา <span style={{ background: "linear-gradient(120deg,#ffba7a,#f47b20)", WebkitBackgroundClip: "text", color: "transparent" }}>มิเตอร์ &amp; หม้อแปลง</span>
          </div>
          <div style={{ fontSize: 14, color: "#c4b5fd", marginBottom: 28, lineHeight: 1.5 }}>ครบในที่เดียว · กฟอ.ฝาง</div>
        </div>

        <div className="auth-form-card" style={{ width: "100%", maxWidth: 420 }}>
      ...
      <style>{`
        @media (max-width: 1024px) {
          .auth-root {
            ...
            background: radial-gradient(160% 130% at 0% 0%, #8b3fc4 0%, #321148 55%, #1b0926 100%) !important;
